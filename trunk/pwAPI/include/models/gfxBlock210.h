...
namespace PW
{
	namespace Models
	{
		...
		#pragma region Read Mesh
							...
							array<PW::Texture::Vertex^, 2>^ points = gcnew array<PW::Texture::Vertex^, 2>(hNumber, wNumber);
							...
							for(int v=0; v<hNumber; v++)
							{
								for(int u=0; u<wNumber; u++)
								{
									...
									points[v, u] = gcnew PW::Texture::Vertex();
		...
		#pragma region Save TEX
							...
							{
								texture = gcnew PW::Texture::Texture();
								texture->Load(fileTexIn, ureverse, vreverse, tilemode, tilecols, tilerows);
							}
							else
							{
							...
							PW::Texture::Texture^ tint = gcnew PW::Texture::Texture();
