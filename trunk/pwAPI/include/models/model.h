...
/*
		...
		// 27 failed
				...
				{
					if(!Directory::Exists(pathIn + enc->GetString(tckPath)))
					{
						// invalid stck path
						return;
		// 1 failed
					...
					else
					{
						...
						stcks = gcnew array<STCK^>(blocks->Length);
						for(int i=0; i<stcks->Length; i++)
						{
							stcks[i] = load_stck(pathIn + enc->GetString(tckPath) + "\\" +  enc->GetString(blocks[i]->stckPath));
						}
						end_sample = blocks[blocks->Length-1]->end_sample;
					}
				}

				// load bon
				BON^ bon = load_bon(pathIn + enc->GetString(bonPath));

				smd = fix_smd(smd);
				bon = fix_bon(bon, stcks, end_sample);

				save_smd(smd, FileOut);
				save_bon(bon, pathOut + enc->GetString(bonPath));
			}
*/
		};
	}
